...
export const GoodsAndSchedule = () => {
  ...
  const datesList = [
    { label: 'Today', sub: 'Immediate Shift', value: 'Today' },
    { label: 'Tomorrow', sub: 'Scheduled', value: 'Tomorrow' },
    { label: 'This Weekend', sub: 'Saturday / Sunday', value: 'Weekend' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1. Goods Category Selection (Matching Exact Requirement from Transmaa Image) */}
      <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-xl border border-slate-200">
        ...
        <div className="mt-4 pt-3 border-t border-slate-100">
          ...
          <input
            ...
            onChange={(e) => setCustomGoodsNote(e.target.value)}
      ...
      <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-xl border border-slate-200">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4">
          {datesList.map((d) => (
            ...
            >
              <div className="text-xs sm:text-sm font-black">{d.label}</div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {timeSlots.map((slot) => (
              ...
              >
                <span>{slot}</span>
                {shiftingTime === slot && <CheckCircle2 className="w-3.5 h-3.5" />}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-xl border border-slate-200">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            { count: 0, label: 'Driver Only', sub: 'Self Loading' },
            { count: 1, label: '1 Helper', sub: '+ ₹350' },
            { count: 2, label: '2 Helpers', sub: '+ ₹700 (Best for 1-2 BHK)' }
          ].map((h) => (
            <button
              key={h.count}
              type="button"
            ...
            >
              <div className="text-xs sm:text-sm font-black">{h.label}</div>
              <div className="text-[10px] text-slate-500 mt-0.5">{h.sub}</div>
            </button>
          ))}
        </div>

        {/* Floor and Elevator toggles if helper is chosen */}
        {helpersCount > 0 && (
